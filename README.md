...
This application is a demo of how to use [SingleStore][singlestore] to deliver personalized ad campaigns to audience segments based on their behavior and realtime location. The demo showcases real-time targeting, audience responsiveness, and campaign performance optimization using location, purchase, and request history from millions of simulated audience segments.
...
The `gen` package is responsible for generating the data. It has a driver which leverages many concurrent goroutines to iterate through all of the audience segments as fast as possible.
