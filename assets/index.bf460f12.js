...
BEGIN
  ...
  END IF;
END`}],we=[{name:"process_locations",createStmt:`CREATE OR REPLACE PROCEDURE process_locations (
    _batch QUERY(
...
END`}],Re=[{name:"worldcities",createStmt:`create rowstore table if not exists worldcities (
  ...
  `,"START PIPELINE worldcities FOREGROUND"],Tr=e=>{const o=Re.find(a=>a.name===e);if(o)return o;const t=we.find(a=>a.name===e);if(t)return t;const r=Oe.find(a=>a.name===e);if(r)return r;throw new Error("Could not find schema object: "+e)},Sr=async e=>{try{return await Ue(e,"SELECT 1"),!0}catch{return!1}},Nr=async e=>{let o={tables:[],procedures:[],functions:[]};const l=e,{database:t}=l,r=x(l,["database"]);try{o=(await Xe(r,`
        ...
        `,t,t)).reduce((s,[c,y])=>(s[c].push(y),s),o)}catch(s){if(!(s instanceof L&&(s.isUnknownDatabase()||s.isDatabaseRecovering())))throw s}const{tables:a,procedures:n,functions:d}=o;return Object.fromEntries([Re.map(({name:s})=>[s,a.includes(s)]),we.map(({name:s})=>[s,n.includes(s)]),Oe.map(({name:s})=>[s,d.includes(s)])].flat())},Or=async e=>_e(e,`
      SELECT COUNT(*) AS count
      FROM information_schema.distributed_partitions
      WHERE
        database_name = ?
        AND role = "Master"
    `,e.database).then(o=>o.count),wr=async(e,{progress:o,scaleFactor:t,includeSeedData:r,skipCreate:a=!1})=>{a||(o("Dropping existing schema","info"),await Ue(e,"DROP DATABASE IF EXISTS `"+e.database+"`"),o("Creating database","info"),await Ue(e,"CREATE DATABASE `"+e.database+"`"));for(const n of Oe)o(`Creating function: ${n.name}`,"info"),await f(e,n.createStmt);for(const n of Re)o(`Creating table: ${n.name}`,"info"),await f(e,n.createStmt);for(const n of we)o(`Creating procedure: ${n.name}`,"info"),await f(e,n.createStmt);await Rr(e),await wo(e,oe),r&&(o("Creating sample data","info"),await Qe(e,oe,t)),o("Schema initialized","success")},Rr=async e=>{for(const o of _r)await f(e,o)},Qe=(e,o,t)=>{const r=100*t.partitions,a=gr(o,r);return sr(e,a)},Lo=async(e,o)=>{const t=o.prefix;return await H(e,`
      SELECT
    ...
    `,e.database)},Co=async(e,o)=>{const t=o.prefix,r=await Lo(e,o);await Promise.all(r.filter(a=>a.needsUpdate).map(async a=>{console.log(`recreating pipeline ${a.pipelineName} for city ${a.cityName}`),a.pipelineName.startsWith("locations_")?await f(e,`
          ...
          `,a.cityId):a.pipelineName.startsWith("purchases_")&&await f(e,`
            ...
            SET ts = NOW(),
              city_id = ?;
          `,a.cityId),await f(e,`ALTER PIPELINE ${a.pipelineName} SET OFFSETS EARLIEST DROP ORPHAN FILES`),await f(e,`START PIPELINE IF NOT RUNNING ${a.pipelineName}`),console.log(`finished creating pipeline ${a.pipelineName} for city ${a.cityName}`)}))},Ir=async e=>{const o=await Xe(e,`
      SELECT
    ...
    `,e.database);await Promise.all(o.map(async([t])=>{console.log("restarting pipeline",t),await f(e,`ALTER PIPELINE ${t} SET OFFSETS EARLIEST DROP ORPHAN FILES`),await f(e,`START PIPELINE IF NOT RUNNING ${t}`)}))},Ar=async e=>{const o=await H(e,`
    ...
    `);return await Promise.all(o.map(({planId:t})=>f(e,`DROP ${t} FROM PLANCACHE`))),o.length>0},Lr=(e,...o)=>{const t=o.map(r=>`"${r}"`).join(",");return No(e,`
      ...
      GROUP BY tableName
    `,e.database)},eo=(e,...o)=>Lr(e,...o).then(t=>t.reduce((r,{tableName:a,count:n})=>(r[a]=n,r),{})),Cr=async(e,o)=>{const{maxRows:t}=o,a=["locations","requests","purchases","notifications"].map(d=>`"${d}"`).join(","),n=await No(e,`
      SELECT
      ...
      WHERE
        ...
        AND stats.count > ?
    `,e.database,t);await Promise.all(n.map(async({tableName:d,count:l,minTs:s,maxTs:c})=>{const u=(l-t)/l;if(u<.2)return;const m=new Date((s+u*(c-s))*1e3);console.log(`removing rows from ${d} older than ${m.toISOString()}`),await f(e,`DELETE FROM ${d} WHERE ts <= ?`,m.toISOString())}))},oo=(e,o="minute")=>_e(e,"ECHO run_matching_process(?)",o).then(t=>t.RESULT),to=e=>_e(e,"ECHO update_segments()").then(o=>o.RESULT),kr=(e,o,t,r)=>Xe(e,`
      SELECT
        ts,
    ...
    `),Dr=(e,o,t)=>_e(e,`
      ...
      LIMIT 1
    `,o,t),A=[{name:"s00",maxRows:1e7,prefix:"v2/100k-2p",partitions:2},{name:"s0",maxRows:2e7,prefix:"v2/100k-4p",partitions:4},{name:"s1",maxRows:4e7,prefix:"v2/100k-8p",partitions:8},{name:"s2",maxRows:16e7,prefix:"v2/1m-16p",partitions:16},{name:"s4",maxRows:32e7,prefix:"v2/1m-32p",partitions:16},{name:"s8",maxRows:64e7,prefix:"v2/1m-64p",partitions:64},{name:"s10",maxRows:1e9,prefix:"v2/10m-80p",partitions:80}],Pr=e=>A.find(o=>o.name===e)||A[0],Mr=e=>[...A].sort((o,t)=>t.partitions-o.partitions).find(o=>o.partitions<=e)||A[A.length-1],Z=({encode:e,decode:o}={encode:JSON.stringify,decode:JSON.parse})=>({setSelf:t,onSet:r,node:a})=>{const n=`recoil.localstorage.${a.key}`,d=localStorage.getItem(n);d!=null&&t(o(d)),r((l,s,c)=>{c?localStorage.removeItem(n):localStorage.setItem(n,e(l))})},ro=()=>({setSelf:e,node:{key:o}})=>{const{location:t}=window;if(t){const r=new URLSearchParams(t.search);e(r.get(o))}},Ur=C({key:"skipCreateDatabase",default:null,effects:[ro()]}),Br=C({key:"sessionId",default:null,effects:[ro()]}),Fr=C({key:"vaporBaseUrl",default:"https://vapor.labs.singlestore.com",effects:[ro()]}),xo=C({key:"connectionHost",default:"http://127.0.0.1",effects:[Z()]}),zo=C({key:"connectionUser",default:"root",effects:[Z()]}),Do=C({key:"connectionPassword",default:"",effects:[Z()]}),X=C({key:"connectionDatabase",default:"martech",effects:[Z()]}),Po=po({key:"vaporConnectionConfig",get:async({get:e})=>{const o=e(Br),t=e(Fr);if(o)try{const r=await fetch(t+"/api/v1/vapor/connect?sessionId="+o);if(r.status===200){const a=await r.json();return{host:a.endpoint,user:a.user,password:a.password,database:e(X)}}}catch{console.log(`Failed to connect to vapor at ${t}, falling back to local config`)}}}),_=po({key:"connectionConfig",get:({get:e})=>{const o=e(Po);if(o)return o;const t=e(xo),r=e(zo),a=e(Do),n=e(X);return{host:t,user:r,password:a,database:n}},cachePolicy_UNSTABLE:{eviction:"most-recent"}}),q=C({key:"configScaleFactor",default:A[0],effects:[Z({encode:e=>e.name,decode:e=>A.find(o=>o.name===e)||A[0]})]}),Q=C({key:"simulatorEnabled",default:!0,effects:[Z()]}),Gr=C({key:"databaseDrawerIsOpen",default:!1}),Ge=nt({key:"tickDurationMs",default:void 0}),Yr=Object.fromEntries([Re.map(({name:e})=>[e,!1]),we.map(({name:e})=>[e,!1]),Oe.map(({name:e})=>[e,!1])].flat()),Mo=(e=!1)=>{const o=v(_);return z(["schemaObjects",o,e],()=>Nr(o),{isPaused:()=>e,refreshInterval:t=>Object.values(t||[]).some(a=>!a)?1e3:0,fallbackData:Yr})},I=()=>{const n=v(_),{database:e}=n,o=x(n,["database"]),t=v(Po),r=z(["isConnected",o],()=>Sr(o)),a=Mo(!r.data);return{isVapor:!!t,connected:!!r.data,initialized:!!r.data&&Object.values(a.data||[]).every(Boolean),reset:()=>{r.mutate(),a.mutate()}}},Hr=(()=>{let e={};return o=>(o in e||(e[o]=1),`${o}(${e[o]++})`)})(),Ye=(e,{name:o,enabled:t,intervalMS:r})=>{const a=dt(Ge(o));g.exports.useEffect(()=>{if(!t)return;const n=new AbortController,d=Hr(o);console.log(`Starting ${d}: tick interval: ${r}ms`);const l=async()=>{try{if(console.time(d),n.signal.aborted)return;const s=performance.now();await e(n);const c=performance.now()-s;a(c),setTimeout(l,Math.max(0,r-c))}catch(s){if(n.signal.aborted||s instanceof L&&s.isUnknownDatabase()||s instanceof DOMException&&s.name==="AbortError")return;throw s}finally{console.timeEnd(d)}};return l(),()=>{console.log(`Stopping ${d}`),n.abort()}},[t,e,r,o,a])},jr=({before:e,after:o,includeSeedData:t})=>{const r=v(_),[a,n]=R(q),{reset:d}=I(),[l,s]=R(Q),c=yo(),y=v(Ur);return g.exports.useCallback(async()=>{const u=l;s(!1),e(),await wr(r,{progress(b,w){const k="reset-schema";c.isActive(k)?c.update(k,{title:b,status:w,duration:w==="success"?2e3:null,isClosable:!0}):c({id:k,title:b,status:w,duration:null})},scaleFactor:a,includeSeedData:t,skipCreate:!!y});const m=await Or(r);n(Mr(m)),o(),d(),s(u)},[l,s,e,r,a,t,y,n,o,d,c])},Uo=(e,o)=>{const[t,r]=g.exports.useState(e);return g.exports.useEffect(()=>{const a=setTimeout(()=>r(e),o);return()=>clearTimeout(a)},[e,o]),t},Bo=()=>{const[{elapsed:e,isRunning:o},t]=g.exports.useReducer((r,a)=>{switch(a.type){case"start":return{start:Math.floor(performance.now()),isRunning:!0,elapsed:r.elapsed};case"stop":return{elapsed:Math.floor(performance.now())-(r.start||0),isRunning:!1}}},{isRunning:!1});return{elapsed:e,isRunning:o,startTimer:()=>t({type:"start"}),stopTimer:()=>t({type:"stop"})}},Fo=(e,o)=>{let t=!1,r=new lt(0,0),a=0;e.on("pointerdown",n=>{t=!0,r=n.data.global,a=performance.now()}),e.on("pointerup",n=>{if(!t||(t=!1,performance.now()-a>200))return;const l=n.data.global;Math.sqrt(Math.pow(l.x-r.x,2)+Math.pow(l.y-r.y,2))>10||o(n)}),e.on("pointercancel",()=>{t=!1})};class Wr extends K{constructor(o,t){super();E(this,"gfx");E(this,"hover",!1);this.city=o,this.onRemove=t,this.gfx=new ge,this.addChild(this.gfx),this.gfx.interactive=!0,this.gfx.on("pointerover",()=>{this.hover=!0}),this.gfx.on("pointerout",()=>{this.hover=!1}),Fo(this.gfx,()=>t(this.city))}update(o){this.gfx.clear(),this.gfx.lineStyle(1,this.hover?16711680:306687,1),this.gfx.beginFill(this.hover?16711680:306687,.2);const[t,r]=o([this.city.centerLat,this.city.centerLon]);this.gfx.drawCircle(t,r,25),this.gfx.endFill()}}const Vr=e=>{const o=v(_),{initialized:t}=I();return z(["cities",o,t],()=>zr(o),{isPaused:()=>!t,onSuccess:e})},Kr=({scene:e,latLngToPixel:o,pixelToLatLng:t,width:r,height:a})=>{const n=v(_),d=co(()=>new K),{mutate:l}=Vr(y=>{d.removeChildren();for(let u=0;u<y.length;u++)d.addChild(new Wr(y[u],c))}),s=g.exports.useCallback(async(y,u)=>{const m=await Dr(n,u,y),b={id:m.id,name:m.name,lonlat:[m.centerLon,m.centerLat],diameter:m.diameter};await wo(n,b),await Qe(n,b,A[0]),l()},[n,l]),c=g.exports.useCallback(async y=>{await ar(n,y.id),l()},[n,l]);return{setup:g.exports.useCallback(()=>{const y=new K;e.addChild(y),e.addChild(d),y.interactive=!0,y.hitArea=new st(0,0,r,a),Fo(y,u=>{const[m,b]=t([u.data.global.x,u.data.global.y]);s(m,b)})},[d,a,s,t,e,r]),update:g.exports.useCallback(()=>{for(let y=0;y<d.children.length;y++)d.children[y].update(o)},[d,o])}},$r=e=>i(Ne,T(h({},e),{useRenderer:Kr,options:{},cursor:"pointer"})),Jr=ct({a:t=>{var r=t,{children:e}=r,o=x(r,["children"]);const{href:a}=o,n=!!(a!=null&&a.startsWith("http"));return n?p(G,T(h({isExternal:n},o),{children:[e,i(pt,{bottom:"2px",boxSize:"0.9em",position:"relative",ml:1})]})):i(G,T(h({to:a||""},o),{as:uo,children:e}))}}),S=t=>{var r=t,{children:e}=r,o=x(r,["children"]);return i(yt,T(h({},o),{skipHtml:!0,components:Jr,children:Array.isArray(e)?e.filter(a=>a).map(a=>ue(a||"")).join(`

`):ue(e)}))},Zr=()=>p($,{maxW:"container.lg",mt:10,mb:"30vh",children:[i($,{maxW:"container.md",mb:10,children:i(S,{children:`
            ## Admin

            You can configure cities and offers on this page. To learn more
            about this application please visit the [Overview page](/).
          `})}),i(Xr,{})]}),Xr=()=>p(N,{children:[i($,{maxW:"container.md",mb:4,children:i(S,{children:`
            #### Cities
            ...
            remove it.
          `})}),i($r,{defaultZoom:4,defaultCenter:[39.716470511656766,-99.59395661915288],height:300})]}),ne=({label:e,placeholder:o,value:t,setValue:r,helpText:a,type:n="text"})=>p(je,{children:[i(We,{mb:1,fontSize:"xs",fontWeight:"bold",textTransform:"uppercase",children:e}),i(go,{size:"sm",placeholder:o,value:t,onChange:d=>r(d.target.value),type:n}),a?i(ut,{fontSize:"xs",children:a}):null]}),qr=()=>{const[e,o]=R(q);return p(je,{children:[i(We,{mb:1,fontSize:"xs",fontWeight:"bold",textTransform:"uppercase",children:"Scale Factor"}),i(gt,{size:"sm",required:!0,value:e.name,onChange:t=>{const r=t.target.value;o(Pr(r))},children:A.map(t=>i("option",{value:t.name,children:t.name},t.name))})]})},Go=({showDatabase:e,showScaleFactor:o})=>{const[t,r]=R(xo),[a,n]=R(zo),[d,l]=R(Do),[s,c]=R(X);return p(J,{spacing:4,children:[i(ne,{label:"Host & Port",placeholder:"http://127.0.0.1:8808",value:t,setValue:r,helpText:i(S,{children:`
              The protocol (http, https), host, and port for the SingleStore
              [HTTP API][1].

              [1]: https://docs.singlestore.com/docs/http-api/
            `})}),p(mo,{columns:2,gap:2,children:[i(ne,{label:"Username",placeholder:"admin",value:a,setValue:n}),i(ne,{label:"Password",placeholder:"",value:d,setValue:l,type:"password"}),e&&i(ne,{label:"Database",placeholder:"martech",value:s,setValue:c}),o&&i(qr,{})]})]})},Yo=e=>i(O,T(h({onClick:()=>{const{pathname:o,hash:t}=window.location;window.location.replace(o+t)},colorScheme:"red"},e),{children:"Disconnect"})),Ho=e=>{const{connected:o,initialized:t}=I(),r=vo(),[a,n]=Ve(),d=v(X),l=te.useRef(null),m=e,{skipSeedData:s,disabled:c}=m,y=x(m,["skipSeedData","disabled"]),u=jr({before:g.exports.useCallback(()=>n.on(),[n]),after:g.exports.useCallback(()=>{n.off(),r.onClose()},[r,n]),includeSeedData:!s});return p(N,{children:[i(O,h({disabled:!o||c,onClick:r.onOpen,colorScheme:t?"green":"red"},y)),i(mt,{isOpen:r.isOpen,onClose:r.onClose,closeOnEsc:!1,closeOnOverlayClick:!1,leastDestructiveRef:l,children:i(Ke,{children:p(vt,{children:[p($e,{fontSize:"lg",fontWeight:"bold",children:[t?"Reset":"Setup"," database ",d]}),p(Je,{children:["This will ",t?"recreate":"create"," the database called ",d,". Are you sure?"]}),p(bo,{children:[i(O,{ref:l,onClick:r.onClose,disabled:a,children:"Cancel"}),i(O,{disabled:a,colorScheme:t?"red":"green",onClick:u,ml:3,children:a?i(U,{}):t?"Recreate database":"Create database"})]})]})})})]})},Qr=({isOpen:e,onClose:o,finalFocusRef:t})=>{const[r,a]=R(Q),{connected:n,initialized:d,isVapor:l}=I();return p(bt,{isOpen:e,placement:"right",onClose:o,finalFocusRef:t,children:[i(Ke,{}),p(ht,{children:[i(ho,{}),i($e,{children:"Config"}),i(Je,{children:p(J,{spacing:4,children:[l?null:i(Go,{showScaleFactor:!0,showDatabase:!0}),p(de,{status:n?"success":"error",borderRadius:"md",children:[i(le,{}),i(se,{children:n?"connected":"disconnected"}),l?i(Yo,{position:"absolute",right:4,top:3,size:"xs"}):null]}),p(de,{status:d?"success":"warning",borderRadius:"md",children:[i(le,{}),i(se,{children:"schema"}),i(Ho,{position:"absolute",right:4,top:3,size:"xs",children:d?"Reset":"Setup"})]}),p(de,{status:r?"success":"warning",borderRadius:"md",children:[i(le,{}),i(se,{children:"simulator"}),i(ft,{position:"absolute",right:4,top:3.5,size:"md",colorScheme:r?"green":"red",isChecked:r,disabled:!n||!d,onChange:()=>a(!r)})]})]})}),i(bo,{})]})]})},ze=({to:e,children:o,onClick:t})=>{const r=wt(e),a=Rt({path:r.pathname,end:!0});return i(G,{as:uo,to:e,px:2,py:1,onClick:t,rounded:"md",_hover:{textDecoration:"none",bg:Me("gray.300","gray.600")},fontWeight:a?"bold":"normal",href:"#",color:Me("gray.700","gray.200"),children:o})},ei=()=>{const{colorMode:e,toggleColorMode:o}=he(),t=vo(),[r,a]=R(Gr),n=te.useRef(null),{connected:d,initialized:l}=I(),s=v(Q),[c]=fo("(max-width: 640px)"),y=p(N,{children:[i(ze,{to:"/",onClick:t.onClose,children:"Overview"}),i(ze,{to:"/map",onClick:t.onClose,children:"Map"}),i(ze,{to:"/admin",onClick:t.onClose,children:"Admin"})]});let u;return c||(u=d?l?s?"connected":"simulator disabled":"needs schema":"disconnected"),i(N,{children:i(P,{bg:Me("gray.200","gray.700"),children:p($,{maxW:"container.lg",children:[p(me,{h:16,alignItems:"center",justifyContent:"space-between",children:[i(xe,{size:"md",icon:t.isOpen?i(Et,{}):i(_t,{}),"aria-label":"Open Menu",display:{md:"none"},onClick:t.isOpen?t.onClose:t.onOpen}),p(ve,{spacing:8,alignItems:"center",children:[i(fe,{as:"h1",size:c?"sm":"md",children:c?"Martech":"Realtime Digital Marketing"}),i(ve,{as:"nav",spacing:4,display:{base:"none",md:"flex"},children:y})]}),p(me,{alignItems:"center",gap:2,children:[p(O,{size:"sm",ref:n,onClick:()=>a(!0),colorScheme:d?l&&s?"green":"yellow":"red",children:[l?i(Eo,{}):i(_o,{}),c||i(Ee,{pl:2,children:u})]}),i(xe,{"aria-label":"Github Repo",size:"sm",icon:e==="light"?i(Tt,{size:"1.4em"}):i(St,{size:"1.4em"}),onClick:()=>window.open("https://github.com/singlestore-labs/demo-realtime-digital-marketing","_blank")}),i(xe,{"aria-label":"Toggle Color Mode",size:"sm",onClick:o,icon:e==="light"?i(Nt,{boxSize:"1.2em"}):i(Ot,{boxSize:"1.2em"})})]})]}),t.isOpen?i(P,{pb:4,display:{md:"none"},children:i(J,{as:"nav",spacing:4,children:y})}):null,i(Qr,{isOpen:r,onClose:()=>a(!1),finalFocusRef:n})]})})})},oi=e=>{const o=g.exports.useRef([]),{data:t}=z(["timeseries",e.name,...e.deps||[]],async()=>{const r=await e.fetcher();return o.current.push({data:r,ts:new Date}),o.current=o.current.slice(-e.limit),o.current},{refreshInterval:e.intervalMS});return t||[]},ti=Ze("~s"),jo=(e,...o)=>oi({name:"estimatedRowCount",deps:o,fetcher:g.exports.useCallback(()=>eo(e,...o),[e,o]),limit:30,intervalMS:1e3}),Wo=t=>{var r=t,{data:e}=r,o=x(r,["data"]);const{colorMode:a}=he(),n=g.exports.useCallback(({tooltipData:s,colorScale:c})=>!c||!s?null:Object.keys(s.datumByKey).sort((y,u)=>s.datumByKey[u].datum.data[u]-s.datumByKey[y].datum.data[y]).map(y=>{const{datum:u}=s.datumByKey[y],m=u.data[y];return p(Ee,{mb:1,color:c(y),fontSize:"sm",children:[y,": ",Ze(".4~s")(m)]},y)}),[]),d=g.exports.useCallback(s=>ti(s).replace("G","B"),[]);if(e.length<2)return i(B,{height:o.height,children:i(U,{size:"md"})});const l=e.length>0?Object.keys(e[0].data).filter(s=>s!=="ts").map(s=>i(It,{dataKey:s,data:e,xAccessor:c=>c==null?void 0:c.ts,yAccessor:c=>c==null?void 0:c.data[s]},s)):null;return p(At,T(h({xScale:{type:"time"},yScale:{type:"sqrt",nice:!0,zero:!1,clamp:!0},theme:a==="light"?Lt:Ct,margin:{left:0,right:50,top:10,bottom:40}},o),{children:[i(no,{orientation:"bottom",numTicks:5,label:"time",labelOffset:10}),i(no,{orientation:"right",numTicks:o.height<250?3:5,tickFormat:d,label:"rows",labelOffset:20}),l,i(kt,{showVerticalCrosshair:!0,detectBounds:!1,offsetLeft:-175,offsetTop:20,renderTooltip:n})]}))},ri=10*1e3,Vo=(e=!1)=>{const o=v(_),t=v(q),r=v(Q),{initialized:a}=I(),n=g.exports.useCallback(d=>{const l=T(h({},o),{ctx:d});return Promise.all([Co(l,t),Ir(l),Ar(l),Cr(l,t),ko(l)])},[o,t]);Ye(n,{name:"SimulatorMonitor",enabled:a&&r&&!e,intervalMS:ri})},ii=1*1e3,ai=10*1e3,ni=()=>{const e=v(_),o=v(Q),{initialized:t}=I(),r=g.exports.useCallback(n=>oo(T(h({},e),{ctx:n}),"minute"),[e]);Ye(r,{name:"SimulatorMatcher",enabled:t&&o,intervalMS:ii});const a=g.exports.useCallback(n=>to(T(h({},e),{ctx:n})),[e]);Ye(a,{name:"SimulatorUpdateSegments",enabled:t&&o,intervalMS:ai})},He=1e3,pe=60*He,ye=60*pe,De=24*ye,D=e=>e.toLocaleString(void 0,{maximumFractionDigits:2}),Y=e=>e?e<He?`${D(e)}ms`:e<pe?`${D(e/He)}s`:e<ye?`${Math.floor(e/pe)}m${Y(e%pe)}`:e<De?`${Math.floor(e/ye)}h${Y(e%ye)}`:`${Math.floor(e/De)}d${Y(e%De)}`:"0s",di=100,li=1e3,F=class extends K{constructor(o){super();E(this,"latlng");E(this,"age",0);E(this,"marker");E(this,"pulse");this.latlng=o,this.marker=new ge,this.marker.beginFill(F.markerColor).drawCircle(0,0,5).endFill(),this.addChild(this.marker),this.pulse=new ge,this.pulse.beginFill(F.pulseColor),this.pulse.drawTorus&&this.pulse.drawTorus(0,0,4,6),this.pulse.endFill(),this.addChild(this.pulse)}update(o,t){if(this.age+=t/60,this.age>F.lifetime&&this.parent){this.parent.removeChild(this);return}const r=this.age%F.lifetime/F.lifetime,a=Pt(r);this.pulse.scale.set(1+a);const n=.4,d=r<n?xt(r/n):1-zt((r-n)/(1-n));this.pulse.alpha=d,r<n?this.marker.alpha=d:this.marker.alpha=1-Dt((r-n)/(1-n));const[l,s]=o(this.latlng);this.x=l,this.y=s}};let ee=F;E(ee,"lifetime",1.5),E(ee,"markerColor",306687),E(ee,"pulseColor",306687);const Ko=()=>{const e=v(_),{initialized:o}=I();return g.exports.useMemo(()=>["notifications",e,o],[e,o])},$o=({scene:e,latLngToPixel:o,bounds:t})=>{const r=g.exports.useRef(new Date().toISOString()),a=v(_),{initialized:n}=I(),d=Uo(t,50),l=Ko();return z(l,()=>kr(a,r.current,di,d),{refreshInterval:li,isPaused:()=>!n,onSuccess:s=>{if(s.length>0){r.current=s[0][0];for(const[,c,y]of s)e.addChild(new ee([y,c]))}}}),{update:g.exports.useCallback(s=>{for(let c=e.children.length-1;c>=0;c--)e.children[c].update(o,s)},[o,e])}},si=()=>{const e=v(_),o=jo(e,"locations","requests","purchases","notifications"),t=z(["notificationsMapTableCounts",e],()=>eo(e,"offers","subscribers","cities","segments"),{refreshInterval:1e3}),r=v(Ge("SimulatorMatcher")),a=v(Ge("SimulatorUpdateSegments")),n=Ze(".4~s"),d=t.data?p(To,{templateColumns:"repeat(auto-fit, minmax(100px, 1fr))",columnGap:2,rowGap:2,children:[p(j,{children:[i(W,{children:"Offers"}),i(V,{children:n(t.data.offers)})]}),p(j,{children:[i(W,{children:"Cities"}),i(V,{children:n(t.data.cities)})]}),p(j,{children:[i(W,{children:"Subscribers"}),i(V,{children:n(t.data.subscribers)})]}),p(j,{children:[i(W,{children:"Segments"}),i(V,{children:n(t.data.segments)})]}),p(j,{children:[i(W,{children:"Segmentation"}),i(V,{children:Y(a)})]}),p(j,{children:[i(W,{children:"Matching"}),i(V,{children:Y(r)})]})]}):null;return p(N,{children:[i(S,{children:`
          ...
          about the demo.
        `}),d,p(P,{children:[i(Ee,{fontSize:"sm",fontWeight:"medium",children:"Row count / time"}),i(Wo,{data:o,height:150})]})]})},ci=()=>{const[e,o]=R(Q);return Vo(),ni(),p(me,{gap:4,justifyContent:"space-between",direction:["column","column","row"],height:"100%",children:[i(J,{spacing:4,flex:"2 2 0",minHeight:"200px",maxHeight:"100%",children:i(Ne,{useRenderer:$o,options:{}})}),p(J,{spacing:4,flex:"1 1 0",minWidth:"0",children:[i(S,{children:`
          ...
          `}),e?i(si,{}):i(N,{children:p(de,{status:"warning",borderRadius:"md",children:[i(le,{}),i(se,{children:"The simulator is disabled"}),i(O,{position:"absolute",right:4,top:3,size:"xs",colorScheme:"blue",onClick:()=>o(!0),children:"Enable simulator"})]})})]})]})},Jo=t=>{var r=t,{children:e}=r,o=x(r,["children"]);return i(Mt,T(h({as:"pre",borderRadius:"md",overflow:"auto",display:"block",px:6,py:4,minW:0},o),{children:e}))},pi=1e3;class yi extends K{constructor(o){super();E(this,"offer");E(this,"points");E(this,"polygon");this.offer=o,this.points=or(o.notificationZone),this.polygon=new ge,this.addChild(this.polygon),this.polygon.interactive=!0,this.polygon.on("mouseover",()=>{this.polygon.tint=65280}),this.polygon.on("mouseout",()=>{this.polygon.tint=16777215})}update(o){this.polygon.clear(),this.polygon.lineStyle(1,306687,1),this.polygon.beginFill(306687,.2),this.polygon.drawPolygon(this.points.flatMap(([t,r])=>o([r,t]))),this.polygon.endFill()}}const ui=({scene:e,latLngToPixel:o,bounds:t})=>{const r=v(_),{initialized:a}=I(),n=Uo(t,100);return z(["offers",r,a,n],()=>xr(r,pi,n),{isPaused:()=>!a,onSuccess:d=>{e.removeChildren();for(let l=0;l<d.length;l++)e.addChild(new yi(d[l]))}}),{update:g.exports.useCallback(()=>{for(let d=0;d<e.children.length;d++)e.children[d].update(o)},[o,e])}},gi=e=>i(Ne,T(h({},e),{useRenderer:ui,options:{}})),M=e=>{const{completed:o,title:t,left:r,right:a}=e,{colorMode:n}=he(),d=n==="light"?".300":".500",l=o?"gray"+d:void 0,s=(o?"green":"gray")+d;return p(N,{children:[p(be,{children:[p(fe,{as:"h2",size:"lg",mb:4,color:l,children:[i(Eo,h({color:s},{boxSize:6,position:"relative",bottom:.5,mr:2})),t]}),r]}),i(be,{children:a})]})},mi=({connected:e,isVapor:o})=>o?i(M,{completed:e,title:"Connected",left:p(N,{children:[i(S,{children:`
            ...
            **Note**: The HTTP API may need to be enabled on your SingleStore
            ...
            [1]: https://docs.singlestore.com/docs/http-api/
          `}),right:i(Go,{})}),vi=({onClose:e,schemaObjectName:o})=>{const t=Tr(o),[r]=fo("(max-width: 640px)");return p(Ft,{isOpen:!0,onClose:e,size:r?"full":"4xl",scrollBehavior:"inside",children:[i(Ke,{}),p(Gt,{children:[p($e,{children:["Create statement for ",t.name]}),i(ho,{}),i(Je,{children:i(Jo,{mb:4,children:t.createStmt})})]})]})},bi=({initialized:e})=>{const[o,t]=R(X),r=Mo(),{colorMode:a}=he(),[n,d]=g.exports.useState(),l=g.exports.useCallback(c=>d(c),[]),s=g.exports.useCallback(()=>d(null),[]);return p(N,{children:[!!n&&i(vi,{onClose:s,schemaObjectName:n}),i(M,{completed:e,title:"Setup the schema",left:p(N,{children:[i(S,{children:`
              ...
              `}),i(Ut,{mt:4,mb:6}),p(ve,{alignItems:"flex-end",children:[p(je,{flex:1,children:[i(We,{fontSize:"xs",fontWeight:"bold",textTransform:"uppercase",children:"Database name"}),i(go,{placeholder:"martech",value:o,size:"sm",onChange:c=>t(c.target.value)})]}),i(P,{flex:1,textAlign:"center",children:i(Ho,{colorScheme:"blue",size:"sm",disabled:e,skipSeedData:!0,children:e?"Schema is setup":"Setup schema"})})]})]}),right:i(mo,{columns:[1,2,2],gap:1,children:Object.keys(r.data||{}).sort().map(c=>{var y;return i(be,{bg:((y=r.data)!=null&&y[c]?"green":"gray")+(a==="light"?".200":".600"),color:a==="light"?"gray.800":"gray.100",textOverflow:"ellipsis",whiteSpace:"nowrap",overflow:"hidden",borderRadius:"md",px:2,py:1,textAlign:"center",_hover:{fontWeight:"bold"},cursor:"pointer",onClick:()=>l(c),children:c},c)})})})]})},Zo=(e,o,t=!0)=>{var n;const r=z(["pipelineStatus",e,o],()=>Lo(e,o),{isPaused:()=>!t}),a=!!((n=r.data)!=null&&n.every(d=>!d.needsUpdate));return{pipelines:r,completed:a}},hi=()=>{const e=v(_),o=v(q),{pipelines:t,completed:r}=Zo(e,o);Vo(!r);const[a,n]=Ve(),d=g.exports.useCallback(async()=>{n.on(),await Co(e,o),t.mutate(),n.off()},[n,e,o,t]),l=jo(e,"locations","requests","purchases"),s=l.length<2||l.every(({data:y})=>y.locations+y.purchases+y.requests===0);return i(M,{completed:r,title:"Ingest data",left:i(S,{children:`
          ...
          `}),right:s||!r?i(B,{h:220,children:p(O,{colorScheme:"blue",size:"sm",onClick:d,disabled:r,children:[(a||r)&&i(U,{mr:2}),a?"Creating Pipelines":r?"...waiting for data":"Create pipelines"]})}):i(Wo,{data:l,height:200})})},Ie=(e,o=!0)=>z(["overviewTableCounts",e],()=>eo(e,"locations","notifications","offers","purchases","requests","segments","subscriber_segments","subscribers"),{isPaused:()=>!o}),fi=()=>{var l,s;const e=v(_),o=v(q),[t,r]=Ve(),a=Ie(e),n=g.exports.useCallback(async()=>{r.on(),await Qe(e,oe,o),a.mutate(),r.off()},[e,o,a,r]),d=!!((l=a.data)!=null&&l.offers);return i(M,{completed:d,title:"Offers",left:i(N,{children:p(S,{children:[`
            ...
            `,d&&`
                ...
                database.
            `]})}),right:d?i(gi,{height:300}):i(B,{h:"100%",children:p(O,{onClick:n,disabled:t,children:[t&&i(U,{mr:2}),t?"loading...":d?"loaded offers!":"load offers"]})})})},Ei=({done:e,setDone:o})=>{const t=v(_);return g.exports.useEffect(()=>{if(e)return;const r=new AbortController,a=T(h({},t),{ctx:r});return(async()=>{try{for(let n=0;n<10;n++){const d=performance.now();if(await to(a),await oo(a,"second"),performance.now()-d<2e3||n>1&&!await ko(a))return}}catch(n){if(r.signal.aborted||n instanceof DOMException&&n.name==="AbortError")return;throw n}})().then(()=>o(!0)),()=>{r.abort()}},[t,e,o]),e?null:i(be,{colSpan:[1,1,2],children:p(B,{w:"100%",h:"200px",color:"gray.500",children:[i(U,{size:"md",mr:4}),i(fe,{size:"md",children:"Warming up queries..."})]})})},_i=()=>{var c;const e=v(_),o=Ie(e),{elapsed:t,isRunning:r,startTimer:a,stopTimer:n}=Bo(),d=!!((c=o.data)!=null&&c.subscriber_segments),l=g.exports.useCallback(async()=>{a(),await to(e),n(),o.mutate()},[e,o,a,n]);let s;if(t&&o.data){const{segments:y,subscriber_segments:u,locations:m,requests:b,purchases:w}=o.data,k=Y(t),Ae=D(m+b+w),Le=D(y),Ce=D(u);s=i(S,{children:`
          The last update evaluated ${Ae} rows against ${Le} segments
          producing ${Ce} segment memberships.
        ...
        `})}return i(M,{completed:d,title:"Segmentation",left:i(S,{children:`
            ...
            Click the button to run the update interactively, or run the following query in your favorite SQL client:
                ...
                select * from dynamic_subscriber_segments;
          `}),right:i(B,{h:"100%",children:p(So,{gap:4,textAlign:"center",children:[p(O,{disabled:r,onClick:l,children:[r&&i(U,{mr:2}),r?"...running":"Match subscribers to segments"]}),s]})})})},Ti=()=>{var b;const e=v(_),o=Ie(e),t=Ko(),{mutate:r}=Bt(),{elapsed:a,isRunning:n,startTimer:d,stopTimer:l}=Bo(),[s,c]=g.exports.useState(0),y=!!((b=o.data)!=null&&b.notifications),u=g.exports.useCallback(async()=>{let w=0,k=0;for(;w===0&&k++<10;)d(),w=await oo(e,"second");l(),c(w),o.mutate(),r(t)},[e,d,l,o,r,t]);let m;if(a&&o.data){const{offers:w,subscribers:k,subscriber_segments:Ae,notifications:Le}=o.data,Ce=D(w*k+Le),Xo=D(Ae),qo=Y(a),Qo=D(s);m=i(S,{children:`
          The last update evaluated up to ${Ce} notification opportunities
        ...
        `})}return i(M,{completed:y,title:"Matching",left:i(S,{children:`
          ...
          `}),right:i(B,{h:"100%",children:p(So,{gap:4,w:"100%",children:[p(O,{disabled:n,onClick:u,children:[n&&i(U,{mr:2}),n?"...running":"Generate notifications"]}),i(P,{width:"100%",children:i(Ne,{height:250,defaultZoom:12,useRenderer:$o,options:{}})}),m]})})})},Si=()=>{const e=v(X);return i(M,{completed:!0,title:"Putting it all together",left:i(S,{children:`
            ...
            * Explore the ${e} database in SingleStore Studio
            ...
            [1]: map
          `}),right:null})},Ni=()=>{const e=v(_),o=v(q),{connected:t,initialized:r,isVapor:a}=I(),{completed:n}=Zo(e,o,t&&r),{data:d}=Ie(e,t&&r),[l,s]=g.exports.useState(!1),c=[{completed:t,component:i(mi,{connected:t,isVapor:a},"connection")},{completed:r,component:i(bi,{initialized:r},"schema")},{completed:n,component:i(hi,{},"pipelines")},{completed:d?d.offers>0:!1,component:i(fi,{},"offers")},{completed:l,component:i(Ei,{done:l,setDone:s},"warmup")},{completed:d?d.subscriber_segments>0:!1,component:i(_i,{},"segmentation")},{completed:d?d.notifications>0:!1,component:i(Ti,{},"matching")},{completed:!0,component:i(Si,{},"summary")}];let y=!0;const u=[];for(const{completed:m,component:b}of c)if(y)u.push(b),y=m;else break;return p($,{maxW:"container.lg",mt:10,mb:"30vh",children:[i(P,{maxW:"container.md",mb:10,px:0,children:i(S,{children:`
          ...
          `})}),i(To,{columnGap:6,rowGap:10,templateColumns:["minmax(0, 1fr)",null,"repeat(2, minmax(0, 1fr))"],children:u})]})};function Oi(){const e=i(B,{height:"100vh",children:i(U,{size:"xl",speed:"0.85s",thickness:"3px",emptyColor:"gray.200",color:"blue.500"})});return i(g.exports.Suspense,{fallback:e,children:p(me,{height:"100vh",direction:"column",children:[i(ei,{}),i(P,{m:4,flex:"1",children:p(Yt,{children:[i(ie,{path:"/",element:i(Ni,{})}),i(ie,{path:"/map",element:i(ci,{})}),i(ie,{path:"/admin",element:i(Zr,{})}),i(ie,{path:"*",element:i(Ht,{to:"/",replace:!0})})]})})]})})}class wi extends te.Component{constructor(){super(void 0);E(this,"state",{});this.handlePromiseRejection=this.handlePromiseRejection.bind(this)}componentDidMount(){window.addEventListener("unhandledrejection",this.handlePromiseRejection)}componentWillUnmount(){window.removeEventListener("unhandledrejection",this.handlePromiseRejection)}handlePromiseRejection(o){this.setState({error:o.reason})}static getDerivedStateFromError(o){return{error:o}}render(){const{error:o}=this.state;if(o){let t;return o instanceof L&&(t=p(N,{children:[i(Ee,{textAlign:"center",children:"An error occurred while running the following query:"}),i(Jo,{children:ue(o.sql)})]})),i($,{maxW:"container.md",my:10,children:p(J,{gap:4,children:[i(B,{children:i(_o,{boxSize:20,color:"red"})}),i(fe,{size:"xl",textAlign:"center",children:o.message}),t,p(ve,{justify:"center",gap:4,children:[i(O,{onClick:()=>this.setState({error:void 0}),size:"sm",children:"Dismiss Error"}),i(O,{onClick:()=>window.location.reload(),size:"sm",colorScheme:"blue",leftIcon:i(jt,{}),children:"Reload"})]})]})})}return this.props.children}}const Ri=Wt({fonts:{heading:"InterVariable, sans-serif",body:"InterVariable, sans-serif",mono:'"Source Code ProVariable", monospace'},components:{Link:{baseStyle:e=>({color:e.colorMode==="light"?"blue.600":"blue.300"})}}}),Ii=({children:e})=>{const o=yo();return i(Zt,{value:{onError:r=>{o({title:"An error occurred",description:r.message,status:"error",duration:5e3,isClosable:!0})}},children:e})};Vt.render(i(te.StrictMode,{children:i(Kt,{theme:Ri,children:i(wi,{children:i(Ii,{children:i($t,{children:i(Jt,{basename:"/",children:i(Oi,{})})})})})})}),document.getElementById("root"));
